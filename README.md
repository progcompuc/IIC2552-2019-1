...
1) Primero intenta resolver el problema por ti mism@, trata de no usar ningún recurso externo excepto por quizá documentación del lenguaje de programación que estás usando (por ej. si es que se te olvidó cómo se usaba cierta cosa de la librería estándar)
2) Si el problema está muy difícil, intenta resolver otro problema más fácil del contest
3) Eventualmente si llegas a punto en que te sientes pegad@ con todos los problemas que quedan, entonces es tiempo de pedir ayuda. Durante la clase te invitamos a discutir el problema con otros compañer@s o con el staff. Fuera de la clase, puedes leer las explicaciones que vayamos subiendo a los google docs o bien puedes hacer preguntas en el Telegram del curso.
...
6) Finalmente, ten presente que vjudge.net le otorga al autor de un contest acceso a las soluciones de los participantes. Sí. Podemos ver sus códigos. Si alguien misteriosamente resuelve un problema difícil, tenga la seguridad de que con probabilidad muy alta su código será examinado, y si se llega a detectar copy-paste, no le irá muy bien.
